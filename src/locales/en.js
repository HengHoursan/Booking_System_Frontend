export default {
  bookings: {
    title: "Booking Management",
    addBooking: "Add Booking",
    createBooking: "Create Booking",
    steps: {
      selectShowtime: "Select Showtime",
      selectSeats: "Select Seats",
      confirm: "Confirm & Pay",
      payment: "Payment",
    },
    screen: "Screen",
    seats: "Seats",
    bookingDetails: "Booking Details",
    noCustomerData: "No customer data",
    bookingSummary: "Booking Summary",
    details: "Details",
    allBookings: "All Bookings",
    searchByMovie: "Search by movie title...",
    searchBookings: "Search bookings...",
    searchByCode: "Search by reference code...",
    bookingId: "Booking ID",
    referenceCode: "Reference Code",
    failed: "Failed",
    customer: "Customer",
    user: "User",
    username: "Username",
    email: "Email",
    phone: "Phone",
    userInfo: "User Information",
    movie: "Movie",
    movieInfo: "Movie Information",
    hall: "Hall",
    showDate: "Show Date",
    showtime: "Showtime",
    showtimeInfo: "Showtime Information",
    startTime: "Start Time",
    endTime: "End Time",
    date: "Date",
    seatsNav: "Seats",
    seatCount: "Seat Count",
    amount: "Amount",
    totalPrice: "Total Price",
    pricing: "Pricing",
    bookingDate: "Booking Date",
    expiryDate: "Expiry Date",
    status: "Status",
    bookingStatus: "Booking Status",
    paymentStatus: "Payment Status",
    paymentId: "Payment ID",
    paymentInfo: "Payment Information",
    notes: "Notes",
    confirmed: "Completed",
    pending: "Pending",
    expired: "Failed",
    completed: "Completed",
    paid: "Paid",
    unpaid: "Unpaid",
    refunded: "Refunded",
    cash: "Cash",
    bakong: "Bakong",
    card: "Card",
    "mobile banking": "Mobile Banking",
    "bank transfer": "Bank Transfer",
    payatcinema: "Pay at Cinema",
    filterByStatus: "Filter by Booking Status",
    filterByPayment: "Filter by Payment Status",
    sortOrder: "Sort Order",
    actions: "Actions",
    view: "View",
    edit: "Edit",
    delete: "Delete",
    deleteSelected: "Delete Selected",
    confirmDelete: "Are you sure you want to delete this booking?",
    confirmDeleteMultiple: "Are you sure you want to delete {count} bookings?",
    deleteSuccess: "Booking deleted successfully",
    deleteMultipleSuccess: "{count} bookings deleted successfully",
    createSuccess: "Booking created successfully",
    updateSuccess: "Booking updated successfully",
    loadFailed: "Failed to load bookings",
    deleteFailed: "Failed to delete booking",
    paymentAndBookingSuccess: "Payment successful and booking created!",
    bookingCreatedPendingPayment: "Booking created and is now pending payment.",
    bakongPaymentFailed:
      "Booking created, but failed to initiate Bakong payment.",
    regenerateQRFailed: "Failed to regenerate QR code.",
    regenerateQRError: "Error regenerating QR code.",
    missingBookingForQR: "Cannot regenerate QR code: Booking ID is missing.",
    paymentPendingMessage:
      "Payment not completed. Booking status set to Pending.",
    noShowtimesFound: "No showtimes found",
    tryAdjustingFilters: "Try adjusting your search or filters",
    clearAllFilters: "Clear all filters",
    occupancy: "Occupancy",
    full: "Full",
    sellingFast: "Selling Fast",
    changeSeat: "Change Seat",
    changeSeatSuccess: "Seats updated successfully!",
    changeSeatFailed: "Failed to update seats.",
    editSeatsWarning:
      "Changing seats may affect the total price. Difference will be calculated based on new seat prices.",
    selectSeatsInstruction: "Please select new seats from the map below.",
    selectedSeats: "Selected Seats",
    maxSeatsError: "A maximum of {count} seats can be booked at a time.",
    sameRowError: "All selected seats must be in the same row.",
    noGapsError:
      "Seats must be continuous. No gaps are allowed between selected seats.",
    samePriceError: "You can only select seats with the same price: ${price}",
    samePriceRestrictionInfo:
      "You can only change to seats with the same price ({price}) as your current booking.",
    confirmedNav: "Completed",
    pendingNav: "Pending",
    expiredNav: "Failed",
    completedNav: "Completed",
    refundedNav: "Refunded",
    paidNav: "Paid",
    unpaidNav: "Unpaid",
    paymentMethod: "Payment Method",
    validation: {
      customerRequired: "Please select a customer to proceed with the booking",
      paymentMethodRequired: "Please select a payment method",
      validationFailed: "Please check all required fields and try again",
    },
    walkinCustomerSelected:
      "Walk-in customer selected (no registration required)",
    createFailed: "Failed to create booking. Please try again.",
  },

  // Navigation and Layout
  my_tickets: "My Tickets",
  upcoming: "Upcoming",
  history: "History",
  no_tickets: "No tickets found",
  no_history: "No booking history found",
  ticket_details: "Ticket Details",
  booking_id: "Booking ID",
  seat: "Seat",
  seatsNav: "Seats",
  hall: "Hall",
  showtime: "Showtime",
  show_date: "Show Date",
  start_time: "Start Time",
  payment_method: "Payment Method",
  total_price: "Total Price",
  ticket_reference: "Ticket Reference",
  tickets_subtitle: "View Booking History",
  seat_type: "Seat Type",
  n_a: "N/A",
  reference: "Reference",
  booking_date: "Booking Date",
  payment_status: "Payment Status",
  actions: {
    close: "Close",
    confirm: "Confirm",
    cancel: "Cancel",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    download_qr: "Save QR Image",
    activate: "Activate",
    deactivate: "Deactivate",
    selectAll: "Select All",
  },
  booking_actions: {
    delete_history: "Remove from History",
    delete_confirm_title: "Remove Booking?",
    delete_confirm_msg:
      "Are you sure you want to remove this booking from your history? This action cannot be undone.",
    delete_success: "Booking removed from history",
    delete_error: "Failed to remove booking",
    delete_all: "Clear History",
    delete_all_confirm_title: "Clear Booking History?",
    delete_all_confirm_msg:
      "Are you sure you want to remove ALL bookings from your history? This action cannot be undone.",
    delete_all_success: "History cleared successfully",
    delete_all_error: "Failed to clear history",
  },
  notifications_i18n: {
    booking_created: {
      title: "Booking Completed",
      message:
        'Your booking {ref} for "{movie}" has been completed.\n\nSeats: {seats}',
    },
    admin_booking_created: {
      title: "New Booking",
      message:
        'New booking {ref} for "{movie}" by {customer}.\n\nSeats: {seats}',
    },
    booking_confirmed: {
      title: "Payment Completed",
      message:
        'Payment received! Your tickets for "{movie}" are completed.\n\nReference: {ref}\nSeats: {seats}\n\nShow this code at the counter or use it to enter.',
    },
    booking_expired: {
      title: "Booking Failed",
      message: "Your booking {ref} has failed.",
    },
    booking_updated: {
      title: "Booking Updated",
      message:
        'Your booking {ref} for "{movie}" has been updated.\n\nSeats: {seats}',
    },
    promotion_new: {
      title: "New Promotion!",
      message: "{promoTitle} (Code: {code})",
    },
    pay_at_cinema: {
      title: "Booking Completed",
      message:
        'Booking {ref} completed for "{movie}".\n\n📌 Important: Please arrive at the cinema at least 30 minutes before the show starts to collect your tickets.\n\nSeats: {seats}',
    },
    pending_payment: {
      title: "Pending Payment",
      message:
        'Booking {ref} created for "{movie}".\n\nPlease complete your {method} payment to confirm your seats.\n\nSeats: {seats}',
    },
    booking_deleted: {
      title: "Booking Deleted",
      message: "Your booking {ref} has been deleted.",
    },
    customer_registered: {
      title: "Registration Successful",
      message: "Welcome {name}! Your account has been registered successfully.",
    },
    general: {
      title: "Notification",
      message: "{message}",
    },
    welcome: {
      title: "Welcome to Movie Booking!",
      message:
        "Hello {name}, welcome to our platform! Enjoy your movie experience.",
    },
  },
  nav: {
    dashboard: "Dashboard",
    users: "Users",
    movies: "Movies",
    bookingsNav: "Bookings",
    settings: "Settings",
    logout: "Logout",
    profile: "Profile",
    reportsNav: "Reports Management",
    clearAll: "Clear All",
    notifications: "Notifications",
    tickets: "My Tickets", // Add this
  },

  // Authentication
  auth: {
    login: "Login",
    logout: "Logout",
    loginTitle: "Cinema Admin",
    loginSubtitle: "Sign in to your admin account",
    phone: "Phone Number",
    phonePlaceholder: "Phone Number",
    usernamePlaceholder: "Username or Email",
    emailPlaceholder: "Enter your email",
    password: "Password",
    passwordPlaceholder: "Password",
    rememberMe: "Remember me",
    signIn: "Sign In",
    loginSuccess: "Login successful!",
    loginFailed: "Login failed",
    invalidCredentials: "Invalid username/email or password",
    logoutSuccess: "Logged out successfully",
    brandingTitle: "Ninja Booking System",
    brandingSubtitle: "",
    demoMode: "Demo Mode",
    apiConnected: "API Connected",
    demoCredentials: "Demo login: +855123456789 / password123",
    apiCredentials: "Connected to backend API. Use your real credentials.",
    checkingApi: "Checking API...",
    testingConnection: "Testing connection to backend API",
    logoutConfirm: {
      title: "Confirm Logout",
      message: "Are you sure you want to logout?",
      confirmButton: "Yes, Log out",
    },
    // Password Reset
    resetPassword: "Reset Password",
    resetPasswordSubtitle: "Enter your phone number to receive a reset code",
    sendResetCode: "Send Reset Code",
    otpSentTitle: "Reset Code Sent",
    otpSentMessage: "A 6-digit reset code has been sent to {phone}",
    otpPlaceholder: "Enter 6-digit code",
    otpRequired: "Reset code is required",
    otpInvalid: "Reset code must be 6 digits",
    resetCodeSent: "Reset code sent successfully",
    resetCodeSendError: "Failed to send reset code",
    resetCodeResent: "Reset code resent successfully",
    didntReceiveCode: "Didn't receive the code?",
    resendCode: "Resend Code",
    resendIn: "Resend in {seconds}s",
    passwordResetSuccess: "Password Reset Successful",
    passwordResetSuccessMessage: "Your password has been reset successfully. You can now login with your new password.",
    passwordResetError: "Failed to reset password",
    rememberPassword: "Remember your password?",
    backToLogin: "Back to Login",
    newPassword: "New Password",
    confirmPassword: "Confirm Password",
  },

  // Validation Messages
  validation: {
    required: "This field is required",
    phoneRequired: "Please input your phone number",
    phoneInvalid: "Please input a valid Cambodian phone number (+855xxxxxxxx)",
    usernameRequired: "Please input your username or email",
    usernameMin: "Username must be at least 2 characters",
    passwordRequired: "Please input your password",
    passwordMin: "Password must be at least 6 characters",
    confirmPasswordRequired: "Please confirm your password",
    passwordMismatch: "Passwords do not match",
    emailRequired: "Please input your email",
    emailInvalid: "Please input a valid email",
    nameLength: "Name must be between 2 and 50 characters",
    verified: "Verified",
    unverified: "Unverified",
  },

  // Dashboard
  dashboard: {
    title: "Dashboard",
    totalUsers: "Total Users",
    totalMovies: "Total Movies",
    totalBookings: "Total Bookings",
    totalRevenue: "Total Revenue",
    seatRevenue: "Seat Type Revenue",
    bookingTrends: "Booking Trends",
    recentActivities: "Recent Activities",
    recentBookings: "Recent Bookings",
    viewAll: "View All",
    thisWeek: "This Week",
    thisMonth: "This Month",
    thisYear: "This Year",
    chartPlaceholder: "Chart will be displayed here",
    revenueOverview: "Revenue Overview",
    bookingStatus: "Booking Status",
    popularMovies: "Popular Movies",
    seatTypeRevenue: "Revenue by Seat Type",
    goodMorning: "Good Morning",
    goodAfternoon: "Good Afternoon",
    goodEvening: "Good Evening",
    activity: {
      system: "System",
      performed: "performed",
      // Action labels
      BOOK_CREATED: "created a booking",
      BOOK_COMPLETED: "completed a booking",
      BOOK_EXPIRE: "expired a booking",
      BOOK_UPDATED: "updated a booking",
      BOOK_DELETED: "deleted a booking",
      BOOK_CREATE_PENDING: "created a booking (pending)",
      BOOK_CREATE_COMPLETED: "completed a booking",
      BOOK_CREATE: "created a booking",
      BOOK_UPDATE: "updated a booking",
      BOOK_DELETE: "deleted a booking",
      USER_CREATED: "created a user",
      USER_UPDATED: "updated a user",
      USER_DELETED: "deleted a user",
      USER_CREATE: "created a user",
      USER_UPDATE: "updated a user",
      USER_DELETE: "deleted a user",
      USER_RESTORE: "restored a user",
      USER_FORCE_DELETE: "permanently deleted a user",
      USER_PASSWORD_RESET: "reset a user password",
      USER_PHONE_UPDATE: "updated a user phone number",
      LOGIN: "logged in",
      LOGOUT: "logged out",
      PAYMENT_CREATED: "created a payment",
      PAYMENT_UPDATED: "updated a payment",
      PAYMENT_CREATE: "created a payment",
      PAYMENT_UPDATE: "updated a payment",
      PAYMENT_DELETE: "deleted a payment",
      MOVIE_CREATED: "created a movie",
      MOVIE_UPDATED: "updated a movie",
      MOVIE_DELETED: "deleted a movie",
      MOVIE_CREATE: "created a movie",
      MOVIE_UPDATE: "updated a movie",
      MOVIE_DELETE: "deleted a movie",
      THEATER_CREATE: "created a theater",
      THEATER_UPDATE: "updated a theater",
      THEATER_DELETE: "deleted a theater",
      HALL_CREATE: "created a hall",
      HALL_UPDATE: "updated a hall",
      HALL_DELETE: "deleted a hall",
      SHOWTIME_CREATED: "created a showtime",
      SHOWTIME_UPDATED: "updated a showtime",
      SHOWTIME_DELETED: "deleted a showtime",
      SHOWTIME_CREATE: "created a showtime",
      SHOWTIME_UPDATE: "updated a showtime",
      SHOWTIME_DELETE: "deleted a showtime",
      SHOWTIME_FORCE_DELETE: "deleted a showtime",
      BOOK_DELETE: "deleted a booking",
      NOTIFICATION_SEND: "sent a notification",
    },
    seatLayoutPreview: "Seat Layout Preview",
    selectShowtimeToPreview: "Select a showtime to preview seats",
    bookNow: "Book Now",
    availableSeats: "Available",
    bookedSeats: "Booked Seats",
    freeSeats: "Free Seats",
    unavailableSeats: "Unavailable",
    totalSeats: "Total",
    totalCapacity: "Total Capacity",
    scheduleTitle: "Showtime Schedule",
    noShowtimesToday: "No showtimes scheduled for this day",
    noSeatsConfigured: "No seats configured for this hall",
    noSeatsHint: "Add seats to this hall first, then they will appear here.",
    goToSeats: "Go to Seat Management",
  },

  // User Management
  users: {
    title: "User Management",
    addUser: "Add User",
    createUser: "Create User",
    editUser: "Edit User",
    allUsers: "All Users",
    customer: "Customer",
    customerDetails: "Customer Details",
    searchUsers: "Search users...",
    filterByStatus: "Filter by status",
    name: "Name",
    username: "Username",
    email: "Email",
    phone: "Phone Number",
    role: "Role",
    status: "Status",
    created: "Created",
    actions: "Actions",
    edit: "Edit",
    delete: "Delete",
    deleteSelected: "Delete Selected",
    confirmDelete: "Are you sure you want to delete this user?",
    confirmDeleteMultiple: "Are you sure you want to delete {count} users?",
    deleteSuccess: "User deleted successfully",
    deleteMultipleSuccess: "{count} users deleted successfully",
    loadError: "Failed to load users",
    createSuccess: "User created successfully",
    updateUser: "Update User",
    updateSuccess: "User updated successfully",
    passwordOptional: "Leave empty to keep current password",
    isVerified: "Verified Status",
    lastLogin: "Last Login",
    active: "Active",
    inactive: "Inactive",
    admin: "Admin",
    superadmin: "Super Admin",
    cashier: "Cashier",
    updated: "Updated",
    createError: "Failed to create user",
    updateError: "Failed to update user",
    loadUserError: "Failed to load user data",
    validation: {
      nameLength: "Name must be between 2 and 50 characters",
      usernameFormat:
        "Username must be 3-30 characters (letters, numbers, underscore only)",
    },
    // Password Reset
    resetPassword: "Reset Password",
    resetPasswordFor: "Reset Password for",
    updatePhone: "Update Phone",
    updatePhoneFor: "Update Phone for",
    currentPhone: "Current Phone",
    newPhone: "New Phone",
    sendNotification: "Send SMS Notification",
    noPhoneForNotification: "No phone number set - SMS notification disabled",
    notificationWillBeSent: "SMS notification will be sent to {phone}",
    phoneFormatTip: "Enter 9 digits (e.g., 12345678 for +85512345678)",
    passwordResetSuccess: "Password reset successfully",
    passwordResetError: "Failed to reset password",
    phoneUpdateSuccess: "Phone number updated successfully",
    phoneUpdateError: "Failed to update phone number",
    phoneNotChanged: "Phone number is the same as current",
    userNotFound: "User not found",
  },
  customers: {
    customer: "Customer",
    title: "Customer Management",
    addCustomer: "Add Customer",
    createCustomer: "Create Customer",
    editCustomer: "Edit Customer",
    allCustomers: "All Customers",
    customerDetails: "Customer Details",
    searchCustomers: "Search customers...",
    searchAndSelectCustomer: "Search and select customer",
    searchExistingCustomer: "Search existing customers",
    newWalkInCustomer: "New Walk-in Customer",
    newGuestCustomer: "New Guest Customer",
    filterByStatus: "Filter by status",
    filterByType: "Filter by type",
    name: "Name",
    email: "Email",
    phone: "Phone Number",
    username: "Username",
    customerType: "Customer Type",
    status: "Status",
    provider: "Provider",
    isVerified: "Is Verified",
    lastLogin: "Last Login",
    restore: "Restore Customer",
    confirmRestore: "Are you sure you want to restore this customer?",
    restoreSuccess: "Customer restored successfully",
    restoreError: "Failed to restore customer",
    showDeleted: "Show Deleted Data",
    deleted: "Deleted",
    passwordChangedAt: "Password Changed At",
    created: "Created",
    actions: "Actions",
    active: "Active",
    inactive: "Inactive",
    member: "Member",
    walkin: "Walk-in",
    guest: "Guest",
    verified: "Verified",
    notVerified: "Not Verified",
    confirmDelete: "Are you sure you want to delete this customer?",
    deleteSuccess: "Customer deleted successfully",
    deleteError: "Failed to delete customer",
    createSuccess: "Customer created successfully",
    createError: "Failed to create customer",
    updateSuccess: "Customer updated successfully",
    updateError: "Failed to update customer",
    loadError: "Failed to load customers",
    guestValidation:
      "Guest customers must have either a phone number or an email.",
  },
  //Seat Managment
  seats: {
    search: "Search Seat",
    indentifier: "Seat Indentifier",
    title: "Seat Management",
    theaterAndHall: "Theater & Hall",
    hallName: "Hall Name",
    theaterName: "Theater Name",
    addSeat: "Add Seat",
    createSeat: "Create Seat",
    deleteSeat: "Delete Seat",
    updateSeat: "Update Seat",
    editSeat: "Edit Seat",
    editSeats: "Edit Seats",
    searchSeats: "Search Seat",
    selectType: "Select Seat Type",
    allSeats: "All Seats",
    seatNumberRange: "Seat Number Range",
    seatRangeStart: "Start (e.g., 1)",
    seatRangeEnd: "End (e.g., 10)",
    seatTotal: "Total: {count} seat(s)",
    row: "Row",
    type: "Type",
    status: "Status",
    price: "Price",
    filterByStatus: "Filter by status",
    filterByRow: "Filter by row",
    filterByTheater: "Filter by Theater",
    filterByHall: "Filter by Hall",
    filterByType: "Filter by type",
    sortOrder: "Sort Order",
    seatDetails: "Seat Details",
    actions: "Actions",
    edit: "Edit",
    delete: "Delete",
    restore: "Restore",
    save: "Save",
    notes: "Notes",
    number: "Seat Number",
    seatNumber: "Seat Number (e.g., 1)",
    deleteSelected: "Delete Selected",
    seatType: "Seat Type",
    created: "Created",
    preview: "Preview",
    types: {
      regular: "Regular",
      vip: "VIP",
      couple: "Couple",
      queen: "Queen",
    },
    statuses: {
      active: "Active",
      maintenance: "Maintenance",
      out_of_order: "Out of Order",
      available: "Available",
      selected: "Selected",
      locked: "Locked (Temporary Hold)",
      closed: "Closed",
      booked: "Booked",
      reserved: "Reserved",
      failed: "Failed",
      canceled: "Canceled",
      unavailable: "Unavailable",
      priceRestricted: "Different Price",
    },
    updateStatus: "Update Status",
    originalInfo: "Original Information",
    multiSeat: "Multi-Seat Mode",
    singleSeatMode: "Single Seat",
    multiSeatMode: "Multi-Seat",
    seatNumbers: "Seat Numbers",
    createError: "Failed to create seat",
    confirmDelete: "Are you sure you want to delete this seat?",
    deleteSuccess: "Seat deleted successfully",
    deleteSelectedConfirm:
      "Are you sure you want to delete  {count} selected seat?",
    forceDeleteTitle: "Permanent Delete Seats",
    forceDeleteSelectedConfirm:
      "<strong>⚠️Warning:</strong> Are you sure you want to <strong>permanently delete</strong> {count} selected seats?<br/><br/>This action <strong>cannot be undone</strong>!",
    forceDeleteSuccess: "Seats permanently deleted successfully",
    forceDeleteFailed: "Failed to permanently delete seats",
    validation: {
      rangeRequired: "Seat range start and end are required.",
      mustBeNumericRange: "Seat range must be numeric for bulk creation.",
      startAfterEnd: "Start of range cannot be greater than end.",
      rangeTooLarge: "The range cannot exceed {max} seats.",
    },
    selectShowtime: "Select show time",
    seatBooking: "Seat booking",
    seatBookingHistory: "Seat Booking History",
    // Duplicate seats
    duplicateToHall: "Duplicate to Hall",
    duplicateSelectedInfo:
      "Duplicate {count} selected seat(s) to another hall.",
    selectedSeats: "Selected Seats",
    fromHall: "From Hall",
    targetHall: "Target Hall",
    selectTargetHall: "Select target hall",
    optionalOverrides: "Optional Overrides",
    keepOriginal: "Keep original",
    keepCurrent: "Keep current",
    duplicate: "Duplicate",
    duplicateSuccess: "{count} seats duplicated successfully",
    duplicateSkipped: "{count} seats skipped (already exist)",
    duplicateError: "Failed to duplicate seats",
    noSeatsSelected: "No seats selected",
    noChangesSelected: "No changes selected",
    // Bulk update
    bulkUpdate: "Bulk Update",
    bulkUpdateInfo: "Update {count} selected seat(s).",
    bulkUpdateSuccess: "{count} seats updated successfully",
    bulkUpdateError: "Failed to update seats",
  },
  // Theater Management
  theaters: {
    theater: "Theater",
    selectTheater: "Select Theater",
    halls: "Halls",
    title: "Theater Management",
    allTheaters: "All Theaters",
    addTheater: "Add Theater",
    createTheater: "Create Theater",
    editTheater: "Edit Theater",
    theaterDetails: "Theater Details",
    name: "Name",
    address: "Address",
    city: "City",
    province: "Province",
    status: "Status",
    price: "Price",
    searchSeats: "Search seats...",
    actions: "Actions",
    edit: "Edit",
    delete: "Delete",
    updateStatus: "Update Status",
    seatNumber: "Seat Number",
    totalHalls: "TotalHalls",
    seatType: "Seat Type",
    availability: "Availability",
    available: "Available",
    unavailable: "Unavailable",
    created: "Created",
    preview: "Preview",
    totalCapacity: "TotalCapacity",
    features: "Features",
    notes: "Notes",
    types: {
      regular: "Regular",
      vip: "VIP",
      couple: "Couple",
      queen: "Queen",
    },
    statuses: {
      active: "Active",
      maintenance: "Under Maintenance",
      out_of_order: "Out of Order",
      reserved: "Reserved",
    },
    updateTheater: "Update Theater",
    confirmDelete: "Are you sure you want to delete this theater?",
    createSuccess: "Theater created successfully.",
    updateSuccess: "Theater updated successfully.",
    deleteSuccess: "Theater deleted successfully.",
    deleteError: "Failed to delete theater.",
    confirmDeleteMultiple:
      "Are you sure you want to delete {count} selected theater(s)?",
    deleteMultipleSuccess: "{count} theaters deleted successfully.",
    deleteMultipleError: "Failed to delete {count} selected theater(s).",
    loadError: "Failed to load theaters.",

  },
  // Hall Management
  halls: {
    hall: "Hall",
    selectHall: "Select Hall",
    title: "Hall Management",
    allHalls: "All Halls",
    addHall: "Add Hall",
    createHall: "Create Hall",
    editHall: "Edit Hall",
    hallDetails: "Hall Details",
    searchHalls: "Search halls...",
    filterByStatus: "Filter by status",
    filterByType: "Filter by screen type",
    filterByTheater: "Filter by Theater",
    sortOrder: "Sort Order",
    name: "Name",
    screenType: "Screen Type",
    type: "Type",
    theater: "Theater",
    totalSeats: "Total Seats",
    totalSeatsNote: "Total Seats (Auto-Calculated)",
    totalSeatsAutoCalculated:
      "Total seats will be automatically calculated based on the actual seats assigned to this hall.",
    totalSeatsCalculatedFromSeats:
      "This value is automatically calculated from the seats assigned to this hall.",
    status: "Status",
    notes: "Notes",
    belongsToTheater: "Belongs to Theater",
    createSuccess: "Hall created successfully",
    updateHall: "Update Hall",
    updateSuccess: "Hall updated successfully",
    deleteError: "Failed to delete hall",
    deleteSuccess: "Hall deleted successfully",
    confirmDelete: "Are you sure you want to delete this hall?",
  },

  // Movie Management
  movies: {
    title: "Movie Management",
    addMovie: "Add Movie",
    createMovie: "Create Movie",
    editMovie: "Edit Movie",
    movieDetails: "Movie Details",
    allMovies: "All Movies",
    searchMovies: "Search movies...",
    filterByStatus: "Filter by status",
    filterByGenre: "Filter by genre",
    filterByLanguage: "Filter by language",
    sortBy: "Sort by",
    movieTitle: "Title",
    genre: "Genre",
    genres: "Genres",
    duration: "Duration",
    durationMinutes: "Duration (minutes)",
    releaseDate: "Release Date",
    endDate: "End Date",
    status: "Status",
    poster: "Poster",
    posterUrl: "Poster URL",
    trailer: "Trailer",
    trailerUrl: "Trailer URL",
    description: "Description",
    rating: "Rating",
    language: "Language",
    languages: "Languages",
    director: "Director",
    cast: "Cast",
    producers: "Producers",
    notes: "Notes",
    nowShowing: "Now Showing",
    comingSoon: "Coming Soon",
    ended: "Ended",
    deleteMovie: "Delete Movie",
    confirmDelete: "Are you sure you want to delete this movie?",
    deleteSuccess: "Movie deleted successfully",
    deleteError: "Failed to delete movie",
    confirmDeleteMultiple: "Are you sure you want to delete {count} movies?",
    deleteMultipleSuccess: "{count} movies deleted successfully",
    deleteMultipleError: "Failed to delete movies",
    createSuccess: "Movie created successfully",
    updateSuccess: "Movie updated successfully",
    loadError: "Failed to load movies",
    statuses: {
      now_showing: "Now Showing",
      coming_soon: "Coming Soon",
      ended: "Ended",
    },
    genreTypes: {
      action: "Action",
      adventure: "Adventure",
      animation: "Animation",
      comedy: "Comedy",
      crime: "Crime",
      documentary: "Documentary",
      drama: "Drama",
      family: "Family",
      fantasy: "Fantasy",
      horror: "Horror",
      mystery: "Mystery",
      romance: "Romance",
      "sci-fi": "Sci-Fi",
      thriller: "Thriller",
      war: "War",
      western: "Western",
    },
  },

  // Promotion Management
  promotions: {
    title: "Promotion Management",
    addPromotion: "Add Promotion",
    createPromotion: "Create Promotion",
    editPromotion: "Edit Promotion",
    promotionDetails: "Promotion Details",
    allPromotions: "All Promotions",
    searchPromotions: "Search by code or title...",
    filterByStatus: "Filter by status",
    sortBy: "Sort by",
    code: "Code",
    titleColumn: "Title",
    startDate: "Start Date",
    endDate: "End Date",
    status: "Status",
    description: "Description",
    image: "Image",
    promotionImage: "Promotion Image",
    deletePromotion: "Delete Promotion",
    confirmDelete: "Are you sure you want to delete this promotion?",
    deleteSuccess: "Promotion deleted successfully",
    deleteFailed: "Failed to delete promotion",
    confirmBulkDelete: "Are you sure you want to delete {count} promotions?",
    deletePromotions: "Delete Promotions",
    bulkDeleteSuccess: "{count} promotions deleted successfully",
    bulkDeleteFailed: "Failed to delete promotions",
    createSuccess: "Promotion created successfully",
    updateSuccess: "Promotion updated successfully",
    loadFailed: "Failed to load promotions",
    statuses: {
      Active: "Active",
      Inactive: "Inactive",
      Expired: "Expired",
    },
  },

  // Showtime Management
  showtimes: {
    title: "Showtime Management",
    addShowtime: "Add Showtime",
    editShowtime: "Edit Showtime",
    showtimeDetails: "Showtime Details",
    searchShowtimes: "Search showtimes...",
    filterByStatus: "Filter by status",
    filterByTheater: "Filter by theater",
    filterByHall: "Filter by hall",
    movie: "Movie",
    theater: "Theater",
    hall: "Hall",
    showDate: "Show Date",
    startTime: "Start Time",
    endTime: "End Time",
    status: "Status",
    pastShowtimes: "Past Showtimes",
    actions: "Actions",
    view: "View",
    edit: "Edit",
    delete: "Delete",
    deleteConfirm: "Are you sure you want to delete this showtime?",
    deleteTitle: "Delete Showtime",
    deleteSuccess: "Showtime deleted successfully",
    deleteFailed: "Failed to delete showtime",
    deleteSelectedConfirm:
      "Are you sure you want to delete {count} selected showtimes?",
    forceDeleteTitle: "Force Delete Showtimes",
    forceDeleteSelectedConfirm:
      "<strong>⚠️ Warning:</strong> Are you sure you want to <strong>permanently delete</strong> {count} selected showtimes?<br/><br/>This action <strong>cannot be undone</strong>!",
    forceDeleteSuccess: "Showtimes permanently deleted successfully",
    forceDeleteFailed: "Failed to permanently delete showtimes",
    loadFailed: "Failed to load showtimes",
    createSuccess: "Showtime created successfully.",
    createMultipleSuccess: "Multiple showtimes created successfully",
    createFailed: "Failed to create showtime.",
    updateSuccess: "Showtime updated successfully",
    updateFailed: "Failed to update showtime",
    created_at: "Created At",
    updated_at: "Updated At",
    allShowtimes: "All Showtimes",
    screen_type: "Screen Type",
    theater_city: "City",
    theater_province: "Province",
    language: "Language",
    subtitle: "Subtitle",
    created_by: "Created By",
    updated_by: "Updated By",
    statuses: {
      scheduled: "Scheduled",
      completed: "Completed",
      expired: "Expired",
      cancelled: "Cancelled",
    },
    createSingle: "Create Single",
    createMultiple: "Create Multiple",
    selectMovie: "Select movie",
    selectTheater: "Select theater",
    selectHall: "Select hall",
    selectShowDate: "Select show date",
    selectStartTime: "Select start time",
    selectEndTime: "Select end time",
    selectStatus: "Select status",
    selectTheaterForAll: "Select theater for all rows",
    limitError: "You can only add up to {max} rows at once.",
    duplicateShowtime: "Duplicate showtimes",
    createMultipleSuccess: "Multiple showtimes created successfully",
    duplicateSuccess: "Duplicate showtimes created successfully",
  },
  validations: {
    movieRequired: "Please select a movie",
    theaterRequired: "Please select a theater",
    hallRequired: "Please select a hall",
    showDateRequired: "Please select a show date",
    startTimeRequired: "Please select a start time",
    endTimeRequired: "Please select an end time",
    statusRequired: "Please select a status",
    movieRequiredShort: "Movie is required",
    hallRequiredShort: "Hall is required",
    dateRequiredShort: "Date is required",
    startTimeRequiredShort: "Start time is required",
    endTimeRequiredShort: "End time is required",
    statusRequiredShort: "Status is required",
  },
  bookingTickets: {
    title: "Booking Tickets",
    search: "Search by ticket",
    searchPlaceholder: "Search by ticket code...",
    filterByTicketType: "Filter by ticket type",
    ticketType: "Ticket Type",
    filterByDate: "Filter by date",
    ticketCode: "Ticket Code",
    user: "User",
    bookingId: "Booking ID",
    movie: "Movie",
    seat: "Seat",
    paymentMethod: "Payment Method",
    issuedAt: "Issued At",
    adult: "Adult",
    child: "Child",
    vip: "VIP",
    standard: "Standard",
    confirmDelete: "Are you sure you want to delete this booking ticket?",
    deleteSuccess: "Booking ticket deleted successfully",
    load_error: "Failed to load booking tickets",
    printTicket: "Print Ticket",
    notFound: "Ticket not found",
    loadError: "Failed to load ticket",
    date: "Date",
    time: "Time",
    hall: "Hall",
    customer: "Customer",
    phone: "Phone Number",
    payment: "Payment",
    price: "Price",
    issued: "Issued",
    termsAndConditions: "Terms & Conditions",
    noRefund: "No refunds or exchanges allowed once ticket is purchased",
    arriveEarly: "Please arrive 15 minutes before showtime",
    validOnce: "This ticket is valid for one-time entry only",
  },
  payments: {
    methods: {
      cash: "Cash",
    },
    cashInstructionsTitle: "Cash Payment",
    cashInstructions:
      "Please collect the exact cash amount from the customer at the counter.",
    bakongPayment: "Bakong Payment",
    payAtCinema: "Pay At Cinema",
    cashPayment: "Cash Payment",
    onlinePayment: "Online Payment",
    createPayment: "Create Payment",
    amount: "Amount",
    paymentDetails: "Payment Details",
    paymentMethod: "Payment Method",
    expiresIn: "Expires In",
    scanToPay: "Scan QR Code with any banking app to pay",
    screenshotInstruction:
      "Please take a screenshot of this QR code to pay via your banking app.",
    paymentExpired: "Payment Expired",
    paymentFailed: "Payment not completed. Your booking has been cancelled.",
    paymentSuccess: "Payment Successful",
    paymentCompleted: "Payment Completed",
    paymentList: "Payment List",
    fetchError: "Failed to fetch payments: ",
    customerName: "Customer Name",
    fromAccount: "From Account",
    toAccount: "To Account",
    paymentDate: "Payment Date",
    paidAt: "Paid At",
    currency: "Currency",
    paid: "Paid",
    unpaid: "Unpaid",
    description: "Description",
    searchPayments: "Search payments...",
    createSuccess: "Payment created successfully.",
    filterByPaymentStatus: "Filter by payment status",
    filterByPaymentMethod: "Filter by payment method",
    filterByPaymentDate: "Filter by payment date",
  },

  // System Management
  system: {
    title: "System Management",
    permissions: "Permissions",
    permissionsDesc: "Manage system-wide access permissions and modules.",
    rolePermissions: "Role Permissions",
    rolePermissionsDesc:
      "Assign and manage permissions for different user roles.",
    allPermissions: "All Permissions",
    noPermissions: "No permissions found",
    manageRoleAssignments: "Manage Role Assignments",
    reload: "Reload",
    module: "Module",
    permission: "Permission",
    displayName: "Display Name",
    name: "Name",
    description: "Description",
    optionalDescription: "Optional description",
    createPermission: "Create Permission",
    createRole: "Create Role",
    exampleEditUsers: "e.g. Edit Users",
    exampleUsersEdit: "e.g. users.edit",
    exampleUsers: "e.g. users",
    exampleManager: "e.g. Manager",
    exampleManagerKey: "e.g. manager",
    accessDeniedAdmin: "Access denied: admin or superadmin required",
    updated: "Updated",
    updateFailed: "Update failed",
    failedToLoadRolePermissions: "Failed to load role permissions",
    filterByModule: "Filter by module",
    searchPermissions: "Search permissions...",
    createSuccess: "Permission created successfully",
    createFailed: "Failed to create permission",
    roleCreateSuccess: "Role created successfully",
    roleCreateFailed: "Failed to create role",
    validationError: "Display name, name and module are required",
    roleValidationError: "Display name and name are required",
  },

  // Settings (Updated for profile page)
  settings: {
    title: "Settings",
    account: "Account Settings",
    personalInfo: "Personal Information",
    personalInfoDesc: "Update your profile and contact details",
    privacySecurity: "Privacy & Security",
    privacySecurityDesc: "Manage your password and security",
    activityHistory: "Activity History",
    activityHistoryDesc: "View your recent activities",
    preferences: "Preferences",
    notifications: "Notifications",
    notificationsDesc: "Manage your notification settings",
    language: "Language",
    languageDesc: "Choose your preferred language",
    appearance: "Appearance",
    theme: "Theme",
    darkMode: "Dark Mode",
    lightMode: "Light Mode",
    themeDesc: "Switch between light and dark modes",
    security: "Security",
    changePassword: "Change Password",
    currentPassword: "Current Password",
    newPassword: "New Password",
    confirmPassword: "Confirm Password",
    telegramConnected: "Telegram Connected",
    general: "General Settings",
    logoutConfirm: {
      title: "Confirm Logout",
      message: "Are you sure you want to sign out?",
      confirmButton: "Sign Out",
    },
    activeSessions: "Active Sessions",
    sessionsDesc: "Devices that are currently logged into your account",
    thisDevice: "This Device",
    logoutOtherDevices: "Logout all other devices",
    deleteAccountWarning: "Danger Zone",
    deleteAccount: "Delete Account",
    deleteAccountDesc:
      "Permanently delete your account and all associated data. This action cannot be undone.",
    deleteAccountConfirmTitle: "Are you sure?",
    deleteAccountConfirmDesc:
      "This will permanently delete your account, your booking history, and all personal data. Are you absolutely sure?",
    confirmDeleteAccount: "Yes, Delete My Account",
    cancel: "Cancel",
    updateError: "Failed to update settings. Please try again.",
    save: "Save",
    updateSuccess: "Profile updated successfully!",
    changePassword: "Change Password",
    currentPassword: "Current Password",
    newPassword: "New Password",
    confirmPassword: "Confirm Password",
    updateProfile: "Update Profile",
    memberSince: "Member Since",
    noneProvided: "None Provided",
    emailPlaceholder: "Enter your email",
    phonePlaceholder: "Enter your phone number",
    sharePhoneTip:
      "To enable automatic phone tracking, please open this app in the Telegram Mini App and choose 'Share Contact'.",
    saving: "Saving Changes...",
    manageAccount:
      "Manage your administrative account settings and view your activity history.",
    profile: "Profile",
    profileDesc: "Update your name, email, and profile photo",
    accountSettings: "Account Settings",
  },

  // Activity Logs
  activity: {
    login: "Login",
    logout: "Logout",
    book_create_pending: "Booking Pending",
    book_create_completed: "Booking Confirmed",
    book_create_confirmed: "Create Booking (Confirmed)",
    book_cancel: "Cancel Booking",
    book_expired: "Booking Expired",
    account_deactivated: "Account Deactivated",
    auth_fail: "Authentication Failed",
    payment_completed: "Payment Completed",
  },

  activity_logs: {
    title: "Activity History",
    noActivities: "No activities yet",
    noActivitiesDesc:
      "Your recent account activities will appear here once you start using the system.",
    pageOf: "Page {page} of {total}",
    actionFailed: "Action Failed",
    reason: "Reason",
    titleDesc: "Your recent administrative and account activities",
    fetchFailed: "Failed to load activity logs",
    columns: {
      action: "Action",
      time: "Time",
      device: "Device",
      ip: "IP Address",
    },
    actions: {
      USER_LOGIN: "Logged In",
      USER_LOGOUT: "Logged Out",
      USER_CREATE: "Created User",
      USER_UPDATE: "Updated User",
      USER_DELETE: "Deleted User",
      USER_RESTORE: "Restored User",
      ROLE_CREATE: "Created Role",
      ROLE_UPDATE: "Updated Role",
      ROLE_DELETE: "Deleted Role",
      PERMISSION_CREATE: "Created Permission",
      PERMISSION_UPDATE: "Updated Permissions",
      PERMISSION_DELETE: "Deleted Permission",
      LOGIN: "Logged In",
      LOGOUT: "Logged Out",
      REGISTER: "Registered",
      PASSWORD_RESET_REQUEST: "Password Reset Requested",
      PASSWORD_RESET: "Password Reset",
      PROFILE_UPDATE: "Updated Profile",
      USER_FORCE_DELETE: "Force Deleted User",
      USER_STATUS_UPDATE: "Updated User Status",
      CUSTOMER_CREATE: "Created Customer",
      CUSTOMER_UPDATE: "Updated Customer",
      CUSTOMER_DELETE: "Deleted Customer",
      CUSTOMER_RESTORE: "Restored Customer",
      CUSTOMER_FORCE_DELETE: "Force Deleted Customer",
      MOVIE_CREATE: "Created Movie",
      MOVIE_UPDATE: "Updated Movie",
      MOVIE_DELETE: "Deleted Movie",
      MOVIE_RESTORE: "Restored Movie",
      MOVIE_FORCE_DELETE: "Force Deleted Movie",
      MOVIE_STATUS_UPDATE: "Updated Movie Status",
      THEATER_CREATE: "Created Theater",
      THEATER_UPDATE: "Updated Theater",
      THEATER_DELETE: "Deleted Theater",
      THEATER_RESTORE: "Restored Theater",
      THEATER_FORCE_DELETE: "Force Deleted Theater",
      HALL_CREATE: "Created Hall",
      HALL_UPDATE: "Updated Hall",
      HALL_DELETE: "Deleted Hall",
      HALL_RESTORE: "Restored Hall",
      HALL_FORCE_DELETE: "Force Deleted Hall",
      SHOWTIME_CREATE: "Created Showtime",
      SHOWTIME_UPDATE: "Updated Showtime",
      SHOWTIME_DELETE: "Deleted Showtime",
      SHOWTIME_RESTORE: "Restored Showtime",
      SHOWTIME_FORCE_DELETE: "Force Deleted Showtime",
      SHOWTIME_STATUS_UPDATE: "Updated Showtime Status",
      SEAT_CREATE: "Created Seat",
      SEAT_UPDATE: "Updated Seat",
      SEAT_DELETE: "Deleted Seat",
      SEAT_RESTORE: "Restored Seat",
      SEAT_FORCE_DELETE: "Force Deleted Seat",
      SEAT_BOOKING_UPDATE: "Updated Seat Booking",
      PROMOTION_CREATE: "Created Promotion",
      PROMOTION_UPDATE: "Updated Promotion",
      PROMOTION_DELETE: "Deleted Promotion",
      PROMOTION_RESTORE: "Restored Promotion",
      PROMOTION_FORCE_DELETE: "Force Deleted Promotion",
      PROMOTION_STATUS_UPDATE: "Updated Promotion Status",
      PAYMENT_CREATE: "Created Payment",
      PAYMENT_UPDATE: "Updated Payment",
      PAYMENT_DELETE: "Deleted Payment",
      PAYMENT_STATUS_UPDATE: "Updated Payment Status",
      PAYMENT_VERIFIED: "Verified Payment",
      INVOICE_GENERATE: "Generated Invoice",
      INVOICE_DELETE: "Deleted Invoice",
      NOTIFICATION_CREATE: "Created Notification",
      NOTIFICATION_UPDATE: "Updated Notification",
      NOTIFICATION_DELETE: "Deleted Notification",
      NOTIFICATION_SEND: "Sent Notification",
      REPORT_GENERATE: "Generated Report",
      FILE_UPLOAD: "Uploaded File",
      FILE_DELETE: "Deleted File",
      BOOK_CREATE_PENDING: "Created Booking (Pending)",
      BOOK_CREATE_COMPLETED: "Created Booking (Completed)",
      BOOK_COMPLETED: "Booking Completed",
      BOOK_UPDATE: "Updated Booking",
      BOOK_EXPIRE: "Expired Booking",
      BOOK_EXPIRE_PENDING: "Booking Expire Pending",
      BOOK_EXPIRE_COMPLETED: "Booking Expire Completed",
      BOOK_RESTORE: "Restored Booking",
      BOOK_DELETE: "Deleted Booking",
      BOOK_FORCE_DELETE: "Force Deleted Booking",
      BOOK_EXPIRED: "Booking Expired",
      BOOK_UPDATE_SEATS: "Updated Booking Seats",
    },
  },

  // Profile
  profile: {
    title: "Profile",
    accountSettings: "Account Settings",
    profileInfo: "Profile Information",
    usernameCannotChange: "Username cannot be changed",
    phoneCannotChange: "Phone number cannot be changed",
    profileUpdated: "Profile updated successfully",
    updateFailed: "Failed to update profile",
    passwordChanged: "Password changed successfully",
    passwordChangeFailed: "Failed to change password",
    passwordsDoNotMatch: "Passwords do not match",
    photoUpdated: "Profile photo updated successfully",
    photoUploadFailed: "Failed to upload profile photo",
    avatarFormatError: "Avatar must be JPG, PNG, or WebP format!",
    avatarSizeError: "Avatar size must be less than 5MB!",
    uploadFailed: "Upload failed",
  },

  // Common Actions
  actions: {
    selectAll: "Select All",
    create: "Create",
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    forceDelete: "Force Delete",
    forceDeleteSelected: "Force Delete Selected",
    save: "Save",
    cancel: "Cancel",
    submit: "Submit",
    reset: "Reset",
    search: "Search",
    filter: "Filter",
    export: "Export",
    import: "Import",
    refresh: "Refresh",
    back: "Go Back",
    next: "Next",
    previous: "Previous",
    close: "Close",
    confirm: "Confirm",
    confirmDelete: "Are you sure you want to delete this item?",
    view: "View",
    watch: "Watch",
    download_qr: "Download QR Code",
    download: "Download",
    downloadPNG: "Download PNG",
    upload: "Upload",
    more: "More",
    remove: "Remove",
    update: "Update",
    addRow: "Add Row",
    deleteSelected: "Delete Selected Items",
    duplicateSelected: "Duplicate Selected Items",
    duplicate: "Duplicate",
    bulkUpdate: "Bulk Update",
    regenerate: "Regenerate",
    today: "Today",
    tomorrow: "Tomorrow",
    print: "Print",
    restore: "Restore",
    markAllAsRead: "Mark All As Read",
    clearAll: "Clear All",
    viewAll: "View All",
    activate: "Activate",
    deactivate: "Deactivate",
    title: "Actions",
  },

  // Status Messages
  messages: {
    success: "Operation completed successfully",
    error: "An error occurred",
    warning: "Warning",
    info: "Information",
    loading: "Loading...",
    noData: "No data available",
    noNotifications: "No new notifications",
    clearAllNotificationsPrompt: "Clear all notifications?",
    clearAllNotificationsWarning:
      "This will permanently remove all your notifications from the list.",
    allNotificationsCleared: "All notifications have been cleared.",
    errorClearingNotifications: "Failed to clear notifications.",
    networkError: "Network error. Please check your connection.",
    serverError: "Server error. Please try again later.",
    httpError: "Http error",
    validationFailed: "Validation failed",
    accessDenied: "Access denied. You don't have permission.",
    sessionExpired: "Session expired. Please login again.",
    accountDeactivated:
      "Your account has been deactivated. Please contact support.",
    tooManyRequests: "Too many requests. Please try again later.",
    loadDataFailed: "Failed to load data",
    deleteFailed: "Failed to delete",
    updateFailed: "Failed to update",
    createFailed: "Failed to create",
    bookingExists:
      "You already have an active booking for this showtime. One customer can only book once per showtime.",
    pendingLimitReached:
      "You have too many pending 'Pay At Cinema' bookings. Please complete or cancel your existing bookings before making a new one.",
    cannotDeleteCompleted: "Completed bookings cannot be deleted.",
    paymentInitiateFailed: "Failed to initiate payment. Please try again.",
    downloadSuccess: "Downloaded successfully",
    downloadFailed: "Failed to download",
    notificationDeleted: "Notification deleted successfully",
    notificationDeleteFailed: "Failed to delete notification",
    qrSaved: "QR Code saved to your device",
    activated: "Account activated successfully",
    deactivated: "Account deactivated successfully",
    confirm: {
      title: "Confirmation",
      activate: "Are you sure you want to activate {name}'s account?",
      deactivate: "Are you sure you want to deactivate {name}'s account?",
    },
  },

  // Table and Pagination
  table: {
    total: "Total",
    showing: "Showing",
    to: "to",
    of: "of",
    entries: "entries",
    noResults: "No results found",
    perPage: "per page",
    page: "Page",
    first: "First",
    last: "Last",
    sortBy: "Sort by",
    selectAll: "Select all",
    selected: "selected",
    actions: "Actions",
  },

  // Date and Time
  datetime: {
    today: "Today",
    yesterday: "Yesterday",
    tomorrow: "Tomorrow",
    thisWeek: "This week",
    thisMonth: "This month",
    thisYear: "This year",
    now: "Now",
    minutesAgo: "{count}m ago",
    hoursAgo: "{count}h ago",
    daysAgo: "{count}d ago",
  },

  // Reports
  reports: {
    title: "Reports",
    managementTitle: "Reports Management",
    subheader:
      "Generate and export detailed data reports for your cinema operations.",
    revenueReport: "Revenue Report",
    revenueDesc: "Detailed financial data including payments and methods.",
    bookingReport: "Booking Report",
    bookingDesc: "Track all customer bookings with status and movie details.",
    moviePerformance: "Movie Performance",
    movieDesc: "Analyze which movies are driving the most revenue.",
    frequencyDesc: "Identify your most loyal customers and their spending.",
    viewReport: "View Report",
    revenueReportDetailed: "Detailed Revenue Report",
    bookingReportDetailed: "Detailed Booking Report",
    moviePerformanceDetailed: "Movie Performance Report",
    export: "Export Data",
    dateRange: "Date Range",
    paymentMethod: "Payment Method",
    bookingRef: "Booking Ref",
    seatsSold: "Seats Sold",
    results: "Results",
    to: "To",
    startDate: "Start Date",
    endDate: "End Date",
    allMethods: "All Methods",
    allStatuses: "All Statuses",
    allMovies: "All Movies",
    totalRecords: "Total Records",
    totalValue: "Total Value",
    totalBookings: "Total Bookings",
    totalRevenue: "Total Revenue",
    movieInfoNote:
      "This report analyzes overall movie performance across all showtimes. It excludes cancelled bookings from the revenue and seat count calculations to provide an accurate picture of actual earnings.",
    method: "Method",
    amount: "Amount",
    ref: "Ref",
    customer: "Customer",
    movie: "Movie",
    status: "Status",
    seats: "Seats",
    total: "Total",
    showtimes: "Showtimes",
    bookings: "Bookings",
    revenue: "Revenue",
    movieTitle: "Movie Title",
    customerFrequency: "Booking Frequency",
    customerName: "Customer Name",
    phone: "Phone",
    email: "Email",
    noDataAvailable: "No customer booking data available",
    date: "Date",
    viewDetails: "View Details",
    paymentMethodAnalysis: "Payment Method Analysis",
    paymentMethodDesc:
      "Analyze payment methods, success rates, and transaction patterns.",
    totalTransactions: "Total Transactions",
    successful: "Successful",
    failed: "Failed",
    pending: "Pending",
    expired: "Expired",
    successRate: "Success Rate",
    failedRate: "Failed Rate",
    revenueContribution: "Revenue Contribution",
    avgTransactionValue: "Avg Transaction Value",
    successfulTransactions: "Successful Transactions",
    failedTransactions: "Failed Transactions",
    expiredTransactions: "Expired Transactions",
    paymentMethodAnalysisNote:
      "This report provides insights into payment method performance, including transaction success rates, revenue contribution by method, and average transaction values. Use this data to optimize payment processing and identify preferred payment methods.",
    search: "Search",
    failedToLoadData: "Failed to load report data",
    firstTransactionDate: "First Transaction Date",
    lastTransactionDate: "Last Transaction Date",
    showtimeUtilization: "Showtime Utilization",
    showtimeUtilizationDesc:
      "Analyze showtime performance, occupancy rates, and revenue per seat.",
    showtimeDate: "Showtime Date",
    startTime: "Start Time",
    endTime: "End Time",
    theater: "Theater",
    hall: "Hall",
    totalSeatsAvailable: "Total Seats Available",
    seatInventoryByHall: "Seat Inventory by Hall",
    seatTypeDistribution: "Seat Type Distribution",
    seatStatusBreakdown: "Seat Status Breakdown",
    seatBookingStatus: "Seat Booking Status",
    seatsBooked: "Seats Booked",
    seatsAvailable: "Seats Available",
    totalSeats: "Total Seats",
    bookedSeats: "Booked Seats",
    availableSeats: "Available Seats",
    occupancyRate: "Occupancy Rate",
    revenuePerSeat: "Revenue Per Seat",
    bookingCount: "Booking Count",
    showtimeUtilizationNote:
      "This report provides insights into showtime performance including occupancy rates, revenue generation, and seat utilization. Use this data to optimize scheduling, pricing, and capacity planning.",
    customerDemographic: "Customer Demographic",
    customerDemographicDesc:
      "Understand customer base composition, age groups, and geographic distribution.",
    ageGroupDistribution: "Age Group Distribution",
    geographicDistribution: "Geographic Distribution",
    newVsReturning: "New vs Returning",
    newCustomers: "New Customers",
    returningCustomers: "Returning Customers",
    growthRate: "Growth Rate",
    customerStatus: "Customer Status",
    active: "Active",
    inactive: "Inactive",
    churned: "Churned",
    retentionRate: "Retention Rate",
    detailedCustomers: "Detailed Customers",
    lastBookingDate: "Last Booking Date",
    totalSpent: "Total Spent",
    ageGroup: "Age Group",
    customerCount: "Customer Count",
    percentage: "Percentage",
    city: "City",
    province: "Province",
    customerType: "Customer Type",
    registrationDate: "Registration Date",
    customerDemographicNote:
      "This report provides insights into customer demographics including age distribution, geographic spread, customer status, and retention metrics. Use this data to understand your customer base and tailor marketing strategies accordingly.",
    staffPerformance: "Staff Performance",
    staffPerformanceDesc:
      "Track staff productivity, revenue generation, and booking metrics.",
    staffName: "Staff Name",
    role: "Role",
    totalBookingsProcessed: "Total Bookings Processed",
    totalRevenueGenerated: "Total Revenue Generated",
    avgBookingValue: "Avg Booking Value",
    completionRate: "Completion Rate",
    expirationRate: "Expiration Rate",
    totalSeatsSold: "Total Seats Sold",
    completedBookings: "Completed Bookings",
    expiredBookings: "Expired Bookings",
    staffPerformanceNote:
      "This report tracks staff performance metrics including bookings processed, revenue generated, completion rates, and expiration rates. Use this data to identify top performers and areas for improvement.",
    inventorySeatManagement: "Inventory & Seat Management",
    inventorySeatManagementDesc:
      "Analyze seat availability, occupancy rates, and seat type distribution.",
    seatType: "Seat Type",
    totalCount: "Total Count",
    avgPrice: "Avg Price",
    count: "Count",
    inventorySeatManagementNote:
      "This report provides insights into seat inventory management including occupancy rates by hall, seat type distribution, pricing analysis, and seat status breakdown. Use this data to optimize seating arrangements and pricing strategies.",
    seatMaintenanceStatus: "Seat Maintenance Status",
    reset: "Reset",
  },

  // Common Fields
  common: {
    createdAt: "Created At",
    updatedAt: "Updated At",
    deletedAt: "Deleted At",
    actions: "Actions",
    loading: "Loading",
    yes: "Yes",
    no: "No",
    none: "None",
    client: "Client",
    admin: "Admin",
    notification: "Notification",
    cancel: "Cancel",
    week: "Week",
    month: "Month",
    year: "Year",
    to: "to",
    startDate: "Start Date",
    endDate: "End Date",
    notSet: "Not Set",
    status: "Status",
  },

  // Upload
  upload: {
    dragDrop: "Drag and drop image here",
    or: "or",
    clickToUpload: "click to upload",
    orUseUrl: "Or use image URL",
    enterImageUrl: "Enter image URL",
    supportedFormats: "Supported formats",
    maxSize: "Max size: {size}MB",
    uploading: "Uploading...",
    uploadSuccess: "Upload successful!",
    uploadFailed: "Upload failed",
    invalidFileType: "Invalid file type",
    fileTooLarge: "File too large. Maximum {size}MB",
    invalidUrl: "Invalid URL format",
  },
  my_tickets: "My Tickets",
  tickets_subtitle: "View your booking history",

  client: {
    home: {
      nowShowingCount: "Now Showing — {count} Movies",
      heroTitle1: "Your Cinema",
      heroTitle2: "Experience Awaits",
      heroDesc:
        "Book your favorite movie tickets instantly at RSB Cinema Erk Phnom. Premium seats, latest movies.",
      bookNow: "Book Now",
      browseMovies: "Browse Movies",
      nowShowing: "Now Showing",
      dontMissOut: "Don't miss out",
      viewAll: "View All",
      location: "Location",
      locationInfo: "Erk Phnom, Phnom Penh<br />Cambodia",
      openHours: "Open Hours",
      openHoursInfo:
        "Daily 9:00 AM – 11:00 PM<br />Weekends open until midnight",
      contact: "Contact",
      contactInfo: "+855 12 345 678<br />info@rsbcinema.com",
      rightsReserved: "© {year} RSB Cinema. All rights reserved.",
      terms: "Terms",
      privacy: "Privacy",
      support: "Support",
    },
    nav: {
      signIn: "Sign In",
      showtimes: "Showtimes",
      cinemaNameKH: "និនចា ប៊ុកឃីង",
      cinemaNameEN: "NINJA BOOKING",
    },
    logout: "Log Out",
    login: {
      welcomeBack: "Welcome Back",
      welcomeUser: "Welcome, {name}!",
      telegramInfo:
        "Please share your contact to continue booking your tickets.",
      signInInfo:
        "Sign in with your Telegram account to explore and book favorite movies.",
      loginWithTelegram: "Login with Telegram",
      loginAsUser: "Log in as {name}",
      fastSecure:
        "Fast, secure login powered by Telegram.<br /> No password or registration required.",
      termsAgreement: "By continuing, you agree to our<br />",
      termsOfService: "Terms of Service",
      privacyPolicy: "Privacy Policy",
      welcomeBackMsg: "Welcome back, {name}!",
      telegramLoginFailed: "Telegram login failed.",
      telegramDataNotFound: "Telegram data not found. Please open in Telegram.",
      serverAuthFailed: "Server authentication failed.",
      telegramSdkError: "Something went wrong with Telegram SDK.",
    },
    register: {
      joinUs: "Join RSB Cinema",
      joinDesc:
        "Create your account and enjoy seamless<br /> movie ticket booking at your fingertips.",
      logIn: "Log In",
      signUp: "Sign Up",
      enterPhone: "Enter your phone number to get started",
      fullName: "Full Name",
      fullNamePlaceholder: "Enter your full name",
      createAccount: "Create Account",
      orContinueWith: "or continue with",
      movies: "Movies",
      halls: "Halls",
      users: "Users",
    },
    seats: {
      screen: "Screen",
      available: "Available",
      selected: "Selected",
      booked: "Booked",
      unavailable: "Unavailable",
      tapToSelect: "Tap a seat to select",
      continue: "Continue",
      seats: "seats",
      seat: "seat",
    },
    showtime: {
      myAccount: "My Account",
      welcomeBack: "Welcome back, {name}",
      todaysShowtime1: "Today's",
      todaysShowtime2: "Showtime",
      sessionsAvailable: "{count} sessions available",
      searchMovies: "Search movies...",
      soldOut: "Sold Out",
      reserveSeats: "Reserve Seats — {title}",
    },
    review: {
      title: "Review Selection",
      selectedSeats: "Selected Seats",
      totalForSeats: "Total for {count} seats",
      addPromoCode: "Add Promo Code",
      continueToPayment: "Continue to Payment",
    },
    checkout: {
      title: "Payment Method",
      bakongName: "Bakong QR",
      bakongDesc: "Pay with any banking app in Cambodia",
      cashName: "Pay at Cinema",
      cashDesc: "Pay and pick up tickets at the counter",
      instant: "Instant",
      flexible: "Flexible",
      toPayToday: "To pay today",
      termsInfo:
        'By clicking "Complete Booking", you agree to our terms and conditions. Electronic tickets will be issued instantly after payment.',
      completeBooking: "Complete Booking",
      processing: "Processing...",
    },
    phoneNumberDialog: {
      title: "Phone Number Required",
      message:
        "Please provide your phone number to continue with the booking. This helps us contact you if needed.",
      placeholder: "e.g., 012 345 678",
      save: "Save & Continue",
      cancel: "Cancel",
      invalidPhone: "Please enter a valid phone number",
      phoneTaken: "This phone number is already taken by another account.",
    },
    confirmation: {
      title: "Booking Confirmed!",
      desc: "Thank you for your purchase. Your booking is now confirmed.",
      bookingRef: "Booking Reference",
      movieSummary: "Movie Summary",
      seats: "Seats",
      totalPrice: "Total Price",
      viewTickets: "View My Tickets",
      backToHome: "Back to Home",
    },
  },

  // Backup Management
  backup: {
    backupSchedule: "Backup Schedule",
    backupRestore: "Backup & Restore",
    backupRestoreDescription: "Manage database backups and restore operations",
    backupScheduleDescription:
      "Configure automated backup schedules using cron expressions",
    title: "Backup Management",
    createBackup: "Create Backup",
    restoreDatabase: "Restore Database",
    newBackupRequest: "New Backup Request",
    createBackupHint:
      "Capture a snapshot of the current database state for safe keeping.",
    criticalAction: "Critical Action",
    scheduleConfiguration: "Schedule Configuration",
    availableBackups: "Available Backups",
    backupName: "Backup Name",
    description: "Description",
    size: "Size",
    created: "Created",
    actions: "Actions",
    status: "Status",
    enabled: "Enabled",
    disabled: "Disabled",
    schedule: "Schedule",
    retention: "Retention",
    lastUpdated: "Last Updated",
    cronExpression: "Cron Expression",
    retentionDays: "Retention Days",
    enableSchedule: "Enable Schedule",
    enableSchedule: "Enable Schedule",
    enableScheduleHelp: "Enable or disable automatic backup scheduling",
    scheduleType: "Schedule Type",
    predefinedSchedule: "Predefined Schedule",
    selectPredefinedSchedule: "Select a predefined schedule",
    customCronExpression: "Custom Cron Expression",
    cronExpressionPlaceholder: "0 2 * * * (Daily at 2 AM)",
    cronFormat: "Format: minute hour day month weekday",
    descriptionPlaceholder: "Enter schedule description...",
    retentionDaysHelp: "Number of days to keep scheduled backups (1-365 days)",
    commonExamples: "Common Examples",
    specialCharacters: "Special Characters",
    anyValue: "Any value",
    listSeparator: "List separator",
    rangeValues: "Range of values",
    stepValues: "Step values",
    everyHour: "Every hour",
    dailyAt2AM: "Daily at 2 AM",
    every6Hours: "Every 6 hours",
    weeklySunday: "Weekly on Sunday",
    weekdays: "Weekdays",
    sunday: "Sunday",
    monday: "Monday",
    tuesday: "Tuesday",
    wednesday: "Wednesday",
    thursday: "Thursday",
    friday: "Friday",
    saturday: "Saturday",
    dailyAtHour: "Daily at {hour}",
    weeklyAtMidnight: "Weekly on {day} at midnight",
    cronExpressionRequired: "Cron expression is required",
    descriptionRequired: "Description is required",
    descriptionLength: "Description must be 1-255 characters",
    retentionDaysRequired: "Retention days is required",
    retentionDaysRange: "Must be between 1-365 days",
    invalidCronFormat: "Invalid cron expression format",
    hour: "Hour",
    day: "Day",
    month: "Month",
    weekday: "Weekday",
    days: "days",
    selectBackup: "Select Backup",
    dropDatabase: "Drop Database",
    warning: "Warning",
    confirmRestore: "Confirm Restore",
    backupDetails: "Backup Details",
    cronHelper: "Cron Expression Helper",
    cronFormat: "Cron Format",
    commonExamples: "Common Examples",
    specialCharacters: "Special Characters",
    saveConfiguration: "Save Configuration",
    reset: "Reset",
    refresh: "Refresh",
    cancel: "Cancel",
    close: "Close",
    delete: "Delete",
    view: "View",
    restore: "Restore",
    type: "Type",
    database: "Database",
    age: "Age",
    path: "Path",
    totalBackups: "Total Backups",
    totalSize: "Total Size",
    scheduled: "Scheduled",
    manual: "Manual",
    searchBackups: "Search backups...",
    noBackupsFound: "No backups found",
    noDescription: "No description",
    currentScheduleStatus: "Current Schedule Status",
    scheduleConfiguration: "Schedule Configuration",
    cronExpressionHelper: "Cron Expression Helper",
    minute: "minute",
    hour: "hour",
    day: "day",
    month: "month",
    weekday: "weekday",
    anyValue: "Any value",
    valueListSeparator: "Value list separator",
    rangeOfValues: "Range of values",
    stepValues: "Step values",
    everyHour: "Every hour",
    dailyAt2AM: "Daily at 2 AM",
    dailyAtMidnight: "Daily at midnight (00:00)",
    dailyAt6AM: "Daily at 6 AM (06:00)",
    dailyAt12PM: "Daily at 12 PM (12:00)",
    dailyAt6PM: "Daily at 6 PM (18:00)",
    every6Hours: "Every 6 hours",
    weeklyOnSunday: "Weekly on Sunday",
    monthlyOn1st: "Monthly on 1st",
    weekdaysAt130AM: "Weekdays at 1:30 AM",
    scheduledBackup: "Scheduled backup",
    customSchedule: "Custom schedule",
    never: "Never",
    mongoToolsRequired: "MongoDB Database Tools Required",
    installationOptions: "Installation Options",
    afterInstallation: "After installation",
    restartTerminal: "Restart your terminal/IDE",
    runCheckCommand: "Run npm run check:mongodump to verify",
    backupSystemRequires:
      "The backup system requires mongodump and mongorestore commands to function",
    messages: {
      enterBackupDescription: "Enter backup description...",
      restoreWarningMessage:
        "This operation will restore the database from the selected backup.",
      chooseBackupToRestore: "Choose a backup to restore",
      yes: "Yes",
      no: "No",
      dropDatabaseHelp:
        "If enabled, the existing database will be completely replaced.",
      backupCreatedSuccessfully: "Backup created successfully",
      databaseRestoredSuccessfully: "Database restored successfully",
      backupDeletedSuccessfully: "Backup deleted successfully",
      scheduleConfigurationSaved: "Schedule configuration saved successfully",
      failedToLoadBackups: "Failed to load backups",
      failedToCreateBackup: "Failed to create backup",
      failedToRestoreBackup: "Failed to restore backup",
      failedToDeleteBackup: "Failed to delete backup",
      failedToSaveSchedule: "Failed to save schedule configuration",
      failedToLoadSchedule: "Failed to load schedule configuration",
      pleaseSelectBackup: "Please select a backup to restore",
      confirmDeleteBackup:
        'Are you sure you want to delete the backup "{name}"? This action cannot be undone.',
      confirmRestoreDatabase:
        "This will restore the database from the selected backup. This action cannot be undone. Continue?",
      cronExpressionRequired: "Cron expression is required",
      invalidCronExpression: "Invalid cron expression format",
      descriptionRequired: "Description is required",
      retentionDaysRequired: "Retention days is required",
      retentionDaysMustBeBetween: "Must be between 1-365 days",
      descriptionMustBeBetween: "Description must be 1-255 characters",
      errorLoadingBackups: "Error loading backups",
      errorCreatingBackup: "Error creating backup",
      errorRestoringBackup: "Error restoring backup",
      errorDeletingBackup: "Error deleting backup",
      errorSavingSchedule: "Error saving schedule configuration",
      errorLoadingSchedule: "Error loading schedule configuration",
      mongoToolsRequired:
        "MongoDB Database Tools are not installed on your system.",
      mongoToolsTitle: "MongoDB Tools Required",
      confirmTitle: "Confirmation",
      deleteTitle: "Confirm Delete",
      restoreTitle: "Confirm Restore",
    },
  },
};
